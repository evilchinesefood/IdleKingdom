...
- **Target:** `johnayers@johndayers.com:/home/johnayers/dev.jdayers.com/kingdom/`. SSH is **password auth only** — pass `-o PreferredAuthentications=password -o PubkeyAuthentication=no` (the SFTP subsystem is blocked; use rsync/scp over ssh). The **Home Server / johndayers.com** password is in the macOS Keychain (service `jdayers-hub`, account `home-server.password`: `security find-generic-password -s jdayers-hub -a home-server.password -w`) — **deliberately not stored in this file**. The deploy toolchain (`sshpass`) is WSL-based; run it from the `/mnt/c/...` path.
